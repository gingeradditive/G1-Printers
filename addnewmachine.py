...
def main():
    parser = argparse.ArgumentParser(
        description="Script per creare una nuova configurazione macchina G1 Rev1"
    )
    ...
    print("=== Creazione nuova configurazione macchina G1 Rev1 ===")
    if args.dry_run:
    ...
    # --- Usa sempre G1 Rev1 (prima entry nel config.json) ---
    keys = list(config.keys())
    if len(keys) == 0:
        sys.exit("❌ Nessuna configurazione trovata in config.json")
    model_name = keys[0]  # Prima entry (G1 Rev1)
